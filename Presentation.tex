...
\begin{document}
{\usebackgroundtemplate{\tikz\node[opacity=0.3, anchor=center]{\includegraphics[width=21cm]{/Users/felixlangot/Google Drive (felixlangot@gmail.com)/UVSQ/Gestion de Projet/Poster/wallpaper2you_568686.jpg}};}
...
\Wider{
    ...
    \begin{itemize}
        ...
        \item Différentes organisation de la convection profonde \autocite{houzeMesoscaleConvectiveSystems2004}
        \item SST $\uparrow$: Organisation + forte \autocite{Khairoutdinov2010}
        \item + d'organisation $\rightarrow$ assèchement de la troposphère \autocite{Tobin2012}
    \end{itemize}
    $\rightarrow$ Potentielle rétroaction climatique \\
    \vspace{3.7cm}
    \begin{tikzpicture}[overlay, remember picture]
        \node[above =1.35cm of current page.south]
        {\includegraphics[width=12cm]{Figures/VisAssemble2.png}};
    \end{tikzpicture}
    \textbf{But du stage:} développer un modèle analytique simple pour comprendre l'effet de l'organisation de la convection sur l'humidité de la troposphère 
}
\end{frame}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=10cm]{Figures/CRM.png}  
\end{figure}
}
\end{frame}
...
\Wider{
    \begin{itemize}
        ...
        \item SAM: $q_c + q_i > 10^{-6}$g/kg alors le point de grille est dans un nuage \autocite{thayer-calderNumericalInvestigationBoundary2015}
    \end{itemize}
}
\begin{figure}[hbtp]
    \centering
...
\begin{frame}{\secname}
    Altitude du nuage le plus proche de chaque point de grille à chaque pas de temps (simulation cumulonimbus):
    \begin{figure}[hbtp]
        \centering
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=7cm]{../Codes/Figs/meanRHpovsRHa.png}
\end{figure}
\vspace{0.2cm}
$\rightarrow$ La $RH$ ne peut pas être prédite par un modèle statique \\ 
$\rightarrow$ Intermittence des nuages + mouvement des parcelles important.
}
\end{frame}
...
\begin{frame}{\secname}
\Wider{
\textbf{Approche dynamique:} \cmark ~mouvement vertical des parcelles, \xmark ~mouvement horizontal, \xmark ~variations en ($x,y,t$) de la vitesse verticale
\vspace{-0.5cm}
\begin{block}{\vspace*{-2.5ex}}
$\rightarrow$ L'altitude de dernière saturation d'une parcelle à 5km est l'altitude à laquelle elle a rencontré un nuage pour la dernière fois le long de sa trajectoire. 
\end{block}
\begin{figure}[hbtp]
    \centering
...
\begin{frame}{\secname}
    \begin{itemize}
        ...
        \item Temps descente des parcelles $\sim$ 10 jours
    \end{itemize} 
    \begin{figure}[hbtp]
        \centering
...
\begin{frame}{\secname}
    En utilisant les mêmes conditions pour détecter les nuages, on obtient les altitudes de dernières saturations suivantes (simulation cumulonimbus):
    \vspace{-0.25cm}
    \begin{figure}[hbtp]
        \centering
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=7cm]{../Codes/Figs/meanRHpvsRHa.png}
\end{figure}
\vspace{0.1cm}
$\rightarrow \overline{RH}$ prédite par le modèle AC correspond à $\overline{RH}$ du CRM \\
$\rightarrow$ hypothèse valide: $\overline{RH}$ prédite sans considérer la microphysique.
}
\end{frame}
...
\vspace{0.1cm}
\centering
\includegraphics[width=11cm]{Figures/SchémaBilan.png}
}
\end{frame}
...
\begin{itemize}
    \setlength\itemsep{7pt}
    \item Importance de la durée de vie des systèmes vs agrégation spatiale
    \item Réinterprétation de la corrélation agrégation spatiale/humidité $\rightarrow$ systèmes + gros = + grande durée de vie
    \item À confirmer avec les observations
    \item Étendre l'expérience sur toute la troposphère
    \item Prédictions des moments d'ordre supérieur de $RH_p \rightarrow$ considérer la microphysique?
\end{itemize}
\end{frame}
